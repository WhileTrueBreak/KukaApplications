...
public class Drawerer_red extends RoboticsAPIApplication{
	@Inject
	private LBR robot;
	@Inject 
...
//		ThreadUtil.milliSleep(10000);
	...
	private void penUp(){
		gripper.move(linRel(
				Drawerer_red.upVector.getX(), 
				Drawerer_red.upVector.getY(), 
				Drawerer_red.upVector.getZ(), 
				World.Current.getRootFrame()).setCartVelocity(25));
	}
	
	private void penDown(){
		gripper.move(linRel(
				Drawerer_red.downVector.getX()-Drawerer_red.upVector.getX(), 
				Drawerer_red.downVector.getY()-Drawerer_red.upVector.getY(), 
				Drawerer_red.downVector.getZ()-Drawerer_red.upVector.getZ(), 
	...
	private void drawPathPlan(PathPlan plan, Frame originFrame, Canvas canvas) {
		...
		for(int i = 0;i < plan.getStartLocs().size();i++) {
			...
			Vector3D first = canvas.toWorld(plan.getStartLocs().get(i)).add(RobotController.frameToVector(originFrame)).add(Drawerer_red.upVector);
	...
	public void run() throws Exception {
		...
		gripper.move(linRel(0,0, -Drawerer_red.PEN_UP_DIST, gripper.getFrame("/TCP")).setJointVelocityRel(0.2));
		...
		Drawerer_red.upVector = originUp.subtract(origin).normalize().multiply(Drawerer_red.PEN_UP_DIST);
		Drawerer_red.downVector = origin.subtract(originUp).normalize().multiply(Drawerer_red.PEN_DOWN_DIST);

		logger.info(String.format("up: %s", originUp.subtract(origin)));
		logger.info(String.format("up: %s", Drawerer_red.upVector.toString()));
		logger.info(String.format("down: %s", Drawerer_red.downVector.toString()));
...
//		logger.info(String.format("Found max at top right: %s", diag.toString()));
		...
		List<String> file = FileReader.readFile(resPath+"/KFlagRed.txt");
