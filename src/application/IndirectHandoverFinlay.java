...
public class IndirectHandoverFinlay extends RoboticsAPIApplication {
	...
	public void run() {
		...
		// part 1
		gripper.move(lin(getApplicationData().getFrame("/PART_1/p1_transition")).setCartVelocity(200));//frame1
		gripper.move(lin(getApplicationData().getFrame("/PART_1")).setCartVelocity(200));//frame1
		gripper2F1.close();
		mF.setLEDBlue(true);
		gripper.move(lin(getApplicationData().getFrame("/PART_1/p1_transition")).setCartVelocity(200));//frame1
		gripper.move(lin(getApplicationData().getFrame("/drop_point/drop_transition")).setCartVelocity(200));//frame1
		gripper.move(lin(getApplicationData().getFrame("/drop_point")).setCartVelocity(200));//frame1
		gripper2F1.open();
		mF.setLEDBlue(true);
