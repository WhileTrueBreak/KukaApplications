...
public class IndirectHandoverFinlay extends RoboticsAPIApplication {
	...
	public void run() {
		...
		// part 1
		gripper.move(ptp(getApplicationData().getFrame("/PART_1/p1_transition")).setJointVelocityRel(0.3));//frame1
		gripper.move(ptp(getApplicationData().getFrame("/PART_1")).setJointVelocityRel(0.3));//frame1
		gripper2F1.close();
		mF.setLEDBlue(true);
		gripper.move(ptp(getApplicationData().getFrame("/PART_1/p1_transition")).setJointVelocityRel(0.3));//frame1
		gripper.move(ptp(getApplicationData().getFrame("/drop_point/drop_transition")).setJointVelocityRel(0.3));//frame1
		gripper.move(ptp(getApplicationData().getFrame("/drop_point")).setJointVelocityRel(0.3));//frame1
		gripper2F1.open();
		mF.setLEDBlue(true);
