 ...
 * @see #dispose()
 */
public class RobotPickAndPlaceTheSequel extends RoboticsAPIApplication {
	@Inject
	private LBR robot;
	...
	public void run() {
		...
		ForceCondition breakForce = ForceCondition.createSpatialForceCondition(gripper.getFrame("/P4"),10);
		int blocks = 3;
		
		for (int i=1;i<=blocks;i++){
			gripper.move(ptp(getApplicationData().getFrame("/P1")).setJointVelocityRel(0.5));
			gripper.move(linRel(0, 0, -30*i, World.Current.getRootFrame()).setCartVelocity(200));//going down
			gripper.move(ptp(getApplicationData().getFrame("/P4")).setJointVelocityRel(0.5));
			gripper.move(linRel(0, 0, -230, World.Current.getRootFrame()).setCartVelocity(200).breakWhen(breakForce));//going down
			gripper.move(ptp(getApplicationData().getFrame("/P4")).setJointVelocityRel(0.5));
		}
		
		
//		gripper.move(ptp(getApplicationData().getFrame("/P1")).setJointVelocityRel(0.5));//frame1
//		gripper.move(linRel(0, 0, -200, World.Current.getRootFrame()).setCartVelocity(200));//testing
//	    gripper.move(linRel(0, 0, 200, World.Current.getRootFrame()).setCartVelocity(200));//testing
...
//		gripper2F1.open();
//		mF.setLEDBlue(false);
//		gripper.move(lin(getApplicationData().getFrame("/P3")).setCartVelocity(200));
//		gripper.move(linRel(0, 0, -200, World.Current.getRootFrame()).setCartVelocity(200));// going down
//		gripper2F1.close();
//		mF.setLEDBlue(true);
//	    gripper.move(lin(getApplicationData().getFrame("/P3")).setCartVelocity(200));
