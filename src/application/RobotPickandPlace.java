...
public class RobotPickandPlace extends RoboticsAPIApplication {
	...
	public void run() {
	    ...
	    gripper.move(linRel(0, 0, -30, World.Current.getRootFrame()).setCartVelocity(20));//going down
		gripper2F1.close();
		mF.setLEDBlue(true);
	    ...
	    gripper.move(linRel(0, 0, -90, World.Current.getRootFrame()).setCartVelocity(20));// going down
		gripper2F1.open();
		mF.setLEDBlue(false);
		...
		gripper.move(lin(getApplicationData().getFrame("/P1")).setCartVelocity(200));//frame1
	    gripper.move(linRel(0, 0, -60, World.Current.getRootFrame()).setCartVelocity(20));//going down
		gripper2F1.close();
		gripper.move(lin(getApplicationData().getFrame("/P1")).setCartVelocity(100));//get back to frame1
		gripper.move(lin(getApplicationData().getFrame("/P2")).setCartVelocity(200));// go to frame2
	    gripper.move(linRel(0, 0, -60, World.Current.getRootFrame()).setCartVelocity(20));// going down
