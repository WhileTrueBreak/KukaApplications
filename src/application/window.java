...
public class window extends RoboticsAPIApplication{
	...
	public void run() {
		...
		lock1.setX(window.getX()-29);
		lock1.setY(window.getY()+14);
		lock1.setZ(window.getZ()-118);
		...
		logger.info("test 1");
		robot.move(ptp(getApplicationData().getFrame("/window/away")).setJointVelocityRel(0.2));
		robot.move(ptp(window).setJointVelocityRel(0.3));
